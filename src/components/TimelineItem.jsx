...
const contentVariant = { // For text content fade-in
  hidden: { opacity: 0, y: 10 },
  visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut", delay: 0.8 } } // Delay after dot
};


const TimelineItem = React.memo(function TimelineItem({ year, title, duration, details }) {
   ...
   return (
      <ol className="flex flex-col md:flex-row relative"> {/* Static container, no border-l here */}
         {/* This li will be the reference for absolute positioning of line and dot */}
         <li className="mb-10 ml-6 relative pl-4"> {/* Added pl-4 to give space for line and dot */}
            ...
            <motion.div variants={contentVariant} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }}>
              <div className="flex flex-wrap gap-x-4 gap-y-1 flex-row items-center justify-start text-xs md:text-sm mb-1">
